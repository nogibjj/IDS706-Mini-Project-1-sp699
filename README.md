...
* __.devcontainer configuration__ for a Python environment
* __Makefile__ with commands for setup, testing, and linting
* __GitHub Actions__ for CI/CD
* __requirements.txt__ for project dependencies
* __README.md__ with setup and usage instructions
...
__`Step 1`__ : Set up with the necessary files to build GitHub Repository such as Makefile, requirements.txt, cicd.yml, Dockerfile, devcontainer.json, etc.</br>
* `Makefile`
...
* `requirements.txt`: Add `Black`(ver=22.3.0), `Pylint`(ver=3.7.1) and `Pytest`(ver=7.1.3) packages</br>
...
* `cicd.yml`: Set up Python(ver=3.11.5), Install packages(by requirements.txt), Run test file, See main Python file, Linting and Formatting</br>
